...
function M.werke()
  ...
  if not handle then
    vim.notify("[hegel.nvim] Text directory not readable: " .. texts_dir, vim.log.levels.ERROR)
    return
  end
  while true do
  ...
  if #werke == 0 then
    vim.notify("[hegel.nvim] No works found in " .. texts_dir, vim.log.levels.WARN)
    return
  end
    ...
    actions = {
      ["default"] = function(selected)
        ...
        if idx then
          ...
          local orig_dir = hegel.config.texts_dir
          hegel.config.texts_dir = werk_dir
          search.search("")
          hegel.config.texts_dir = orig_dir
        end
      end,
    },
  })
...
function M._werke_select(werke, texts_dir)
  vim.ui.select(werke, {
    ...
    local search = require("hegel.search")
    local orig_dir = hegel.config.texts_dir
    hegel.config.texts_dir = texts_dir .. "/" .. choice
    search.search("")
    hegel.config.texts_dir = orig_dir
  end)
end
...
function M.zufall()
  ...
  if #files == 0 then
    vim.notify("[hegel.nvim] No text files found.", vim.log.levels.WARN)
    return
  end
  ...
  if #content_lines == 0 then
    vim.notify("[hegel.nvim] Empty file: " .. file, vim.log.levels.WARN)
    return
  end
